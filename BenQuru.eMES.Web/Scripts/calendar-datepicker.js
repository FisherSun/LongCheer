 ...
 */
 ...
 $.extend(Calendar.prototype,{
	...
	_renderCalendarPanel: function(){
		...
		// 面板及背景遮挡层插入到页面中
		date_pane.appendTo(self.options.wrap);

		// 创建遮罩层的目地是：只显示一个日历面板
		var block_bg = $("<div class='cxcalendar_lock'></div>").appendTo(self.options.wrap);
	...
	_renderBody: function(y,m){
			...
			for(var j = 0; j < 7; j++) {
				...
				if(typeof(day_num) == 'number') {
					...
					if(cur_m > text_m || cur_m < text_m) {
					    disabled = 'disabled';
					    temp_html += (" title='" + y + options.separator + (tempM+1) + options.separator + (day_num < 10 ? "0" + day_num : day_num) + "' class='num " + disabled + "");
					}else {
					    disabled = "";
					    temp_html += (" title='" + y + options.separator + (tempM < 10 ? "0" + tempM : tempM) + options.separator + (day_num < 10 ? "0" + day_num : day_num) + "' class='num " + disabled + "");
					}
				...
				temp_html += (" data-day='" + day_num + "'  style='vertical-align:middle;'>" + day_num + "</td>");
				//temp_html += (" data-day='" + day_num + "' style='vertical-align:middle;font-size:16px'>" + day_num + "</td>");
		...
		self.options.afterChange(y + "-" + ((m + 1) < 10 ? "0" + (m + 1) : (m + 1)));
		return this;
	},
