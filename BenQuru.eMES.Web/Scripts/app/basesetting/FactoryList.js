    ...
    function (app, router, s, utility, rm, dic, pm, user) {
        var f = function () {
            ...
            this.Search = function () {
                ...
                if (self.objectValue.FACCODE())
                    self.bodylist.Condition.push("FACCODE=" + self.objectValue.FACCODE());
            ...
            this.bodylist = new s('api/Factory', '', self.tableSettings, true);
            ...
            this.Save = function () {
                ...
                if (self.keyValue() == 'new') {
                    ...
                    utility.httpPost('/api/Factory', d).done(function (data) {
                        app.showMessage(data, self.rm.message.alertTitle());
                        $("#myModal").modal("hide");
                        self.ClearText();
                        self.selectedItem.removeAll();
            ...
            this.Del = function (item) {
                ...
                app.showMessage(self.rm.message.ConfirmDelete() + "?", self.rm.message.confirmationTitle(), [
                    { text: self.rm.button.buttonConfirm(), value: "Yes" }, { text: self.rm.button.buttonCancel(), value: "No" }]).then(function (r) {
                        if (r === 'Yes') {
                            $.isLoading();
                            utility.httpDelete('api/Factory/Delete/?UserCode=' + self.selectedItem()[0]['USERCODE']())
                                .done(function (data) {
            ...
            this.Export = function () {
                if (self.bodylist.TotalCount() == 0) {
                    ...
                    var url = '/api/Factory?isExport=true&' + appix.join('&');
