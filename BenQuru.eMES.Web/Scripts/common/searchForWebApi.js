define(['jsRuntime/resourceManager', 'jsRuntime/utility'], function (rm, utility) {
    ...
    var f = function (target, orderby, settings, ClearAfterPageChange) {
        var self = this;
        this.rm = rm.global;
        ...
        this.Search = function (includeCount) {
            ...
            if (self.Condition.length > 0)
                for (var i = 0; i < self.Condition.length; i++) {
                    appix.push(self.Condition[i]);
                }
            var countUrl = self.Target + 'Count' + (appix.length > 0 ? "?" + appix.join('&') : "");
            appix.push('count=' + self.RowCount());
            if (self.Skip > 0 && includeCount)
                appix.push('skip=' + self.Skip);
            appix.push('orderby=' + self.OrderBy());
            var qUrl = self.Target + '?' + appix.join('&');
            utility.httpGet(qUrl).done(function (data) {
                ko.mapper.fromJS(data.Total, {}, self.TotalCount);
                ko.mapper.fromJS(data.Rows, {}, self.Result);
